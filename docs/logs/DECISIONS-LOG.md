...
| 2026-06-28 | Task 6 Middleware | register spatie aliases (`role`, `permission`, `role_or_permission`) ใน `bootstrap/app.php` ผ่าน `$middleware->alias()` | register ใน RouteServiceProvider หรือ HTTP Kernel | Laravel 13 ใช้ Application::configure ไม่มี Kernel; `alias()` คือวิธีมาตรฐาน |
| 2026-06-28 | Task 6 Middleware | ใช้ `permission:access admin` บน route group — return 403 (HttpException ของ spatie) | สร้าง custom middleware redirect ไปหน้าอื่น | DoD ระบุแค่ 403 หรือ redirect; spatie throw 403 อยู่แล้ว ไม่ต้อง custom (YAGNI invariant #10) |
| 2026-06-28 | Task 7 SoftDelete | `profile.destroy` ที่ user ลบตัวเองเป็น soft delete (ไม่ใช่ hard delete) — test อัปเดตให้ assert ว่า `User::find()` = null แต่ `withTrashed()->find()` ยังอยู่ | ให้ destroy() ใช้ forceDelete() เพื่อไม่พัง test เดิม | invariant #7 ชัดเจนว่า User (master data) ต้อง soft delete; hard delete ละเมิด invariant |
| 2026-06-28 | Task 7 SoftDelete | `$instance->fresh()` บน soft-deleted model คืน object (bypass global scope) แตกต่างจาก `Model::find()` ที่คืน null | ใช้ fresh() ใน assertion | ทดสอบจริงผ่าน tinker พบว่า fresh() ใช้ withTrashed ภายใน ต้องใช้ User::find() แทนใน assertion |
